...
class QQMLMODELS_EXPORT QQmlObjectListSortFilterModelBase
    : public QSortFilterProxyModel { // abstract Qt base class
    Q_OBJECT
...
class /*QQMLMODELS_EXPORT*/ QQmlObjectListModel : public QQmlObjectListSortFilterModelBase {
  public:
    explicit QQmlObjectListModel(QObject *parent = Q_NULLPTR,
                                 const QByteArray &displayRole = QByteArray(),
                                 const QByteArray &uidRole = QByteArray())
        : QQmlObjectListSortFilterModelBase(parent), m_count(0), m_uidRoleName(uidRole),
          m_dispRoleName(displayRole), m_metaObj(ItemType::staticMetaObject) {
        ...
        for (int propertyIdx = 0, role = (baseRole() + 1); propertyIdx < len;
            ...
            if (!roleNamesBlacklist.contains(propName)) {
                ...
                qWarning() << "Can't have" << propName << "as a role name in"
                           << qPrintable(QString::fromLatin1(CLASS_NAME));
            }
        }
    }
  ...
  public: // C++ API
    ...
    void move(int idx, int pos) {
        if (idx != pos) {
            // FIXME : use begin/end MoveRows when supported by Repeater, since then use
            // remove/insert pair
  ...
  protected: // internal stuff
    ...
    void dereferenceItem(ItemType *item) {
        if (item != Q_NULLPTR) {
            ...
            if (item->parent() ==
                this) { // FIXME : maybe that's not the best way to test ownership ?
                item->deleteLater();
            }
        }
    }
    void onItemPropertyChanged(void) {
        ...
        if (row >= 0 && role >= 0) {
            QModelIndex index = QSortFilterProxyModel::index(row, 0, noParent());
...
#define QML_OBJSORTFILTERMODEL_PROPERTY(type, name)                                                \
  protected:                                                                                       \
    Q_PROPERTY(QQmlObjectListSortFilterModelBase *name READ get_##name CONSTANT)                   \
  private:                                                                                         \
    QQmlObjectListSortFilterModel<type> *m_##name;                                                 \
                                                                                                   \
  public:                                                                                          \
    QQmlObjectListSortFilterModel<type> *get_##name(void) const {                                  \
        return m_##name;                                                                           \
    }                                                                                              \
                                                                                                   \
  private:
