...
class QQMLMODELS_EXPORT QQmlObjectListSortFilterModelBase : public QSortFilterProxyModel  { // abstract Qt base class
    Q_OBJECT
    Q_PROPERTY (int count READ count NOTIFY countChanged)

public:
    explicit QQmlObjectListSortFilterModelBase (QObject * parent = Q_NULLPTR) : QSortFilterProxyModel  (parent) { }
...
template<class ItemType> class /*QQMLMODELS_EXPORT*/ QQmlObjectListModel : public QQmlObjectListSortFilterModelBase {
public:
    explicit QQmlObjectListModel (QObject *          parent      = Q_NULLPTR,
                                  const QByteArray & displayRole = QByteArray (),
                                  const QByteArray & uidRole     = QByteArray ())
        : QQmlObjectListSortFilterModelBase (parent)
        , m_count (0)
...
#define QML_OBJSORTFILTERMODEL_PROPERTY(type, name) \
    protected: Q_PROPERTY (QQmlObjectListSortFilterModelBase * name READ get_##name CONSTANT) \
    private: QQmlObjectListSortFilterModel<type> * m_##name; \
    public: QQmlObjectListSortFilterModel<type> * get_##name (void) const { return m_##name; } \
    private:
